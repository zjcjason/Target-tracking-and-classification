...
def draw_real(_list_img, _list_loc, _list_loc_temp, _i, _j, _k):
    ...
    # if _k == 0:
    #     cv2.putText(_list_img[_i], "1", _list_loc[_j][2:],
    #                 cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255), 2)
    # if _k == 1:
    #     cv2.putText(_list_img[_i], "2", _list_loc[_j][2:],
    #                 cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255), 2)
    # if _k == 2:
    #     cv2.putText(_list_img[_i], "3", _list_loc[_j][2:],
    #                 cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255), 2)
...
def list_to_array(_list_pre_temp, _list_loc_temp):
    ...
    # print("_list_pre_temp:", _list_pre_temp)
    # print("array_pre_temp_xy", array_pre_temp_xy)
    # print("array_pre_temp_x1y1", array_pre_temp_x1y1)
    ...
    # print("list_loc_temp:", _list_loc_temp)
    # print("array_loc_temp_xy", array_loc_temp_xy)
    # print("array_loc_temp_x1y1", array_loc_temp_x1y1)
...
# def kf_pre(_row_ind, _col_ind, _list_kf, _list_loc_temp, _list_pre_temp):
#     for ind in range(len(_row_ind)):
#         if ind >= len(_list_kf):  # 因为代价矩阵初始化为6*6，所以索引必为1*6。避免越界
#             continue
#         _list_pre_temp[_col_ind[ind]] = _list_kf[_col_ind[ind]].predict(_list_loc_temp[_row_ind[ind]])
...
# 保留KF
# def life(_time_2, _time_3, _list_pre_temp):
#     if _time_2 > 5 & len(_list_pre_temp) > 2:
#         # del list_kf[2:4]
#         _list_pre_temp[2] = [0, 0]
#         _list_pre_temp[3] = [0, 0]
...
# def draw_pre(_list_img, _list_num, _i, _list_pre_temp):
#     for draw in range(_list_num[_i]):
#         if draw == 0:
...
def main():
    root = 'C:/Users/10462/OneDrive/Graduation Project/code/jupyter/'
    list_kf = []
    list_img, list_cnt = data.process_list(root, 0, 1500)  # 获得所有的原图列表和轮廓列表
    ...
    for cnts in list_cnt:
        ...
        for cnt in cnts:
            if data.cnt_area(cnt) < 100:
                continue
            # print(k, len(list_loc_temp), len(list_pre_temp), len(list_kf) / 2, list_num[i])
        ...
        # print("list_loc_temp:", list_loc_temp)
        ...
        # print("iou:", iou_single)
        list_iou.append(iou_single)
        # print("row_ind:", row_ind)
        # print("col_ind:", col_ind)
        # print("list_loc_temp:", list_loc_temp)
        # print("iou<0.7 list_pre_temp:", list_pre_temp)
        ...
        # life(time_2, time_3, list_pre_temp)  # 保留KF
        life(time_2, time_3, list_pre_temp, list_kf)  # 不保留KF
        # draw_pre(list_img, list_num, i, list_pre_temp)
        draw_pre(list_img, col_ind, i, list_pre_temp, list_loc_temp)
        # print("after kf list_pre_temp:", list_pre_temp)
        time_2 += 1
        time_3 += 1
        if list_num[i] > 1:
            time_2 = 0
        if list_num[i] > 2:
            time_3 = 0
        # data.show(list_img, i)
        data.save(root+'new/', i, list_img)
        # print(i)
        i += 1
        # print("--------")
    # print(list_iou)
    ...
    # np.save('list_iou_50%_3.npy', list_iou_save)
    # plt.plot(list_iou)
    # plt.title("1500帧50%_3结果")
    # plt.xlabel("帧数")
    # plt.ylabel("IOU")
    # plt.savefig('list_iou_50%_3.jpg')
    # plt.show()
